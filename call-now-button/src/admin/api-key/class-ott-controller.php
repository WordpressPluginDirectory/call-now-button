...
class OttController {
    ...
    /**
     * Called when the activation link is opened while logged out of WordPress
     * (`admin_post_nopriv_cnb_apikey_activate`).
     *
     * Without this handler WordPress ends the request with an empty page. Instead, send the visitor
     * to wp-login.php, with `redirect_to` pointing back at the original activation URL (including the OTT),
     * so they land straight back on activation after logging in.
     *
     * @return void
     */
    public function redirect_to_login() {
        // wp_login_url() is generated by WordPress itself, so a plain redirect is safe here
        wp_redirect( wp_login_url( $this->get_activation_url() ) ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
        exit;
    }

    /**
     * Rebuild the activation URL as it was requested, query arguments included.
     *
     * @return string
     */
    public function get_activation_url() {
        $args = array();
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- The activation link is a plain GET link from an e-mail
        foreach ( wp_unslash( $_GET ) as $key => $value ) {
            if ( is_string( $value ) ) {
                // add_query_arg() does not encode newly added values, so do it here
                $args[ sanitize_key( $key ) ] = rawurlencode( sanitize_text_field( $value ) );
            }
        }
        // Always make sure the activation action is part of the URL
        $args['action'] = 'cnb_apikey_activate';
    ...
    private function get_api_key() {
        ...
        $api_key_direct = sanitize_text_field( filter_input( INPUT_GET, 'api_key' ) );
        $api_key_ott    = sanitize_text_field( filter_input( INPUT_GET, 'api_key_ott' ) );
